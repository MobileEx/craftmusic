...
const DisplayChatUserRow = ({ user, message, onPressUser }) => {
  ...
  async function getUserImage() {
    ...
    const res = await ProfileService.getUserProfile(user.key);
  ...
  return (
    <View style={[STYLES.horizontalAlign]}>
      <TouchableOpacity onPress={onPressUser}>
        <FastImage
          // source={user.avatar ? { uri: `${Environment.S3URL}/${user.avatar}` } : userimage}
          source={
            user.avatar
              ? {
...
export default DisplayChatUserRow;
