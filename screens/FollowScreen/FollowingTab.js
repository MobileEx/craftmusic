...
class FollowingTab extends React.Component {
    ...
    ProfileService.getFollow(this.state.userId)
      .then((res) => {
        ...
        for (let i = 0; i < res.data.followings.length; i++) {
          followingStatus.push(false);
          for (let j = 0; j < this.myFollowingList.length; j++) {
            if (this.myFollowingList[j].id == res.data.followings[i].id) {
              followingStatus[i] = true;
              break;
            }
          }
        }
        ...
        this.setState({
          followingList: res.data.followings,
          followingStatus,
        });
      })
      .catch((err) => {
  ...
  onFollow = (id) => {
    const { followingStatus, followingList } = this.state;
    const statusFlags = followingStatus;
    const followUsers = followingList;
    if (statusFlags[id]) {
    ...
    this.setState({ followingStatus: statusFlags });
  };
  ...
  render() {
    const { nameFilterText, followingStatus, followingList } = this.state;
    return (
