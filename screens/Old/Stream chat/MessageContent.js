...
const Container = styled.TouchableOpacity`
  ...
  background-color: ${({ error, theme }) =>
    error ? theme.message.content.errorContainer.backgroundColor : theme.colors.transparent};
  border-bottom-left-radius: ${({ alignment, theme }) =>
    alignment === 'left' ? theme.message.text.borderRadiusS : theme.message.text.borderRadiusL};
  border-bottom-right-radius: ${({ alignment, theme }) =>
    alignment === 'left' ? theme.message.text.borderRadiusL : theme.message.text.borderRadiusS};
...
class MessageContent extends React.PureComponent {
  ...
  openThread = () => {
    if (this.props.onThreadSelect) this.props.onThreadSelect(this.props.message);
  };
  ...
  _setReactionPickerPosition = () => {
    ...
    this.messageContainer.measureInWindow((x, y, width) => {
      this.setState({
        ...
        rpRight:
          pos === 'right' ? Math.round(Dimensions.get('window').width) - (x + width + 10) : null,
      });
    });
  };
  ...
  render() {
    ...
    const hasReactions =
      reactionsEnabled && message.latest_reactions && message.latest_reactions.length > 0;

    const options = [{ id: 'cancel', title: 'Cancel' }];
    const images =
      hasAttachment &&
      message.attachments.filter(
        (item) => item.type === 'image' && !item.title_link && !item.og_scrape_url
      );

    const files = hasAttachment && message.attachments.filter((item) => item.type === 'file');
    ...
    if (messageActions && messageActions.indexOf(MESSAGE_ACTIONS.edit) > -1 && canEditMessage())
      options.splice(1, 0, {
        id: MESSAGE_ACTIONS.edit,
        title: 'Edit Message',
      });

    if (messageActions && messageActions.indexOf(MESSAGE_ACTIONS.delete) > -1 && canDeleteMessage())
      options.splice(1, 0, {
    ...
    return (
      <MessageContentContext.Provider value={context}>
        ...
        >
          {message.type === 'error' ? <FailedText>ERROR · UNSENT</FailedText> : null}
          {message.status === 'failed' ? <FailedText>Message failed - try again</FailedText> : null}
          {reactionsEnabled && message.latest_reactions && message.latest_reactions.length > 0 && (
            <ReactionList
              position={pos}
          ...
          {!MessageFooter && showTime ? (
            <MetaContainer>
              <MetaText alignment={pos}>{moment(message.created_at).format('h:mmA')}</MetaText>
            </MetaContainer>
          ) : null}
