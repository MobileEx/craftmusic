...
// console.log('show the attachment imported', Avatar);
...
class MessageSimple extends React.PureComponent {
  ...
  render() {
    const { message, isMyMessage, groupStyles, forceAlign, showMessageStatus } = this.props;

    let pos;
    if ((forceAlign && forceAlign === 'left') || forceAlign === 'right') pos = forceAlign;
    ...
    const isVeryLastMessage = lastMessage ? lastMessage.id === message.id : false;
    const hasMarginBottom = !!(groupStyles[0] === 'single' || groupStyles[0] === 'bottom');
    ...
    return (
      ...
      >
        {pos === 'right' ? (
          <>
            <Text style={{ color: 'white' }}>wow text right</Text>
          ...
          <>
            <Text style={{ color: 'white' }}>left side text</Text>
            {/* <MessageAvatar {...this.props} /> */}
            {/* <MessageContent {...this.props} alignment={pos} /> */}
          </>
        )}
