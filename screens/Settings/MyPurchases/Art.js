...
class Sales extends React.Component {
  ...
  componentDidMount() {
    ProfileService.getMyPurchase(this.state.userId, 1)
      .then((res) => {
        this.setState({
          salesItems: res.data,
        });
      ...
      .then(async (res) => {
        ...
        if (status === 200) {
          ...
          const shareOptions = {
            ...
            failOnCancel: false,
            saveToFiles: true,
            url: res.path(), // base64 with mimeType or path to local file
          };
  ...
  setCraftMedia = (purchaseId) => {
    PlayingCraftService.setCraftMediaPurchase(this.props.craftId, purchaseId, '1')
      .then((res) => {
        console.log(res);
                ...
                <View key={item.id} style={styles.item}>
                  ...
                  <View style={styles.rightContent}>
                    ...
                    <View style={{ ...STYLES.horizontalAlign, justifyContent: 'space-between' }}>
                      ...
                      >
                        <IconButton iconName="upload-copy" title="Export" />
                      </TouchableOpacity>
                    </View>
                  </View>
                </View>
